...
EOF
    ...
    local A=""
    local i
    for i in `cat $T/build/envsetup.sh $T/vendor/aos/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      A="$A $i"
    done
    echo $A
}
...
function build_build_var_cache()
{
    ...
    cached_vars=`cat $T/build/envsetup.sh $T/vendor/aos/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
    cached_abs_vars=`cat $T/build/envsetup.sh $T/vendor/aos/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^aos_") ; then
        AOS_BUILD=$(echo -n $1 | sed -e 's/^aos_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $AOS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        AOS_BUILD=
    fi
    export AOS_BUILD
...
function lunch()
{
    ...
    if [ "$1" ] ; then
        ...
        tput setaf 2;
        tput bold;
        echo -n "Go ahead and pick a number or enter lunch combo(aos_device-userdebug)... "
        tput sgr0;
        read answer
    fi
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the TeamDarkness-Devices git
        T=$(gettop)
        cd $T > /dev/null
        vendor/aos/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/aos/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
. $ANDROID_BUILD_TOP/vendor/aos/build/envsetup.sh
